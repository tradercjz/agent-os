下面给你整理一个**产品级单机高性能本体图计算存储框架**的完整 Markdown 说明文档，可直接交给团队参考或用于设计评审：
...
1. 分块读取原始数据
2. 实体抽取 & 关系抽取
3. ID 映射 (string → uint64)
4. 分块排序 & 外部归并
5. 构建 CSR + 倒排索引
6. 序列化到磁盘，生成 snapshot
...
1. 只读数据保证 CSR 高效
2. 高度节点（super-node）可单独二级索引
3. 动态频繁写入需用 LSM 或分区策略
4. 实体 ID 连续、避免随机写
5. 构建阶段必须排序、压缩、批量

---

文档完成后，你的 C++ 团队可以直接按目录和模块实现，Phase 1 就能构建单机亿级边图，并支持快速查询和子图抽取，为后续 GraphRAG 业务打下基础。

---

如果你需要，我可以帮你直接画出**C++ 类结构图 + 文件二进制布局图 + Builder 流程图**，便于团队快速落地。
