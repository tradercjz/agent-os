...
struct MemoryEntry {
    ...
    // 简单 TF-IDF 式嵌入（词频向量，维度 = 常用词表大小）
    static Embedding naive_embed(std::string_view text, size_t dim = 64) {
        Embedding vec(dim, 0.0f);
        // 将文本哈希到固定维度（简单 n-gram 方法）
        for (size_t i = 0; i < text.size(); ++i) {
            uint32_t h = 0;
            for (size_t j = i; j < std::min(i + 4, text.size()); ++j) {
                h = h * 31 + static_cast<unsigned char>(text[j]);
            }
            vec[h % dim] += 1.0f;
        }
        // L2 归一化
...
public:
    ...
    // 语义检索 Top-K
    virtual Result<std::vector<SearchResult>>
    search(std::string_view query, size_t top_k = 5) = 0;
...
public:
    ...
    Result<std::string> write(MemoryEntry entry) override {
        ...
        if (entry.id.empty()) entry.id = "wm_" + std::to_string(id_counter_++);
        entry.created_at = entry.accessed_at = now();
        entry.embedding  = MemoryEntry::naive_embed(entry.content);

        // LRU 驱逐
        if (store_.size() >= capacity_) {
            auto oldest = std::min_element(store_.begin(), store_.end(),
    ...
    search(std::string_view query, size_t top_k = 5) override {
        std::lock_guard lk(mu_);
        auto q_emb = MemoryEntry::naive_embed(query);
    ...
    size_t size() const override {
        std::lock_guard lk(mu_); return store_.size();
    }
    std::string name() const override { return "WorkingMemory"; }
    void clear() { std::lock_guard lk(mu_); store_.clear(); }
...
public:
    ...
    Result<std::string> write(MemoryEntry entry) override {
        std::lock_guard lk(mu_);
        if (entry.id.empty()) entry.id = "st_" + std::to_string(id_counter_++);
        entry.created_at = entry.accessed_at = now();
        entry.embedding  = MemoryEntry::naive_embed(entry.content);

        if (store_.size() >= capacity_) {
            // 驱逐重要性最低、最久未访问的条目
            auto victim = std::min_element(store_.begin(), store_.end(),
                [](const auto& a, const auto& b) {
                    float score_a = a.second.importance /
                                    (1.0f + a.second.access_count);
                    float score_b = b.second.importance /
                                    (1.0f + b.second.access_count);
    ...
    Result<bool> forget(const std::string& id) override {
        std::lock_guard lk(mu_); return store_.erase(id) > 0;
    }
...
public:
    ...
    Result<std::string> write(MemoryEntry entry) override {
        if (entry.id.empty()) entry.id = "lt_" + std::to_string(id_counter_++);
        entry.created_at = entry.accessed_at = now();
        entry.embedding  = MemoryEntry::naive_embed(entry.content);
    ...
    search(std::string_view query, size_t top_k = 5) override {
        ...
        for (auto& [score, id] : scored) {
            auto entry = read(id);
            if (entry) results.push_back({*entry, score});
        }
        return results;
    }

    size_t size() const override {
        std::lock_guard lk(mu_); return index_.size();
    }
...
private:
    ...
    void load_index() {
        ...
        while (std::getline(ifs, line)) {
            ...
            ls >> rec.id >> rec.importance;
            index_[rec.id] = rec;
            // embedding 在搜索时重建（naive embed from file）
        }
    }
...
public:
    ...
    Result<std::string> remember(std::string content,
    ...
    // 检索：L0 → L1 → L2 逐层查找
    Result<std::vector<SearchResult>>
    recall(std::string_view query, size_t top_k = 5) {
        std::vector<SearchResult> results;

        // L0 工作记忆
        if (auto r = working_->search(query, top_k)) {
            for (auto& sr : *r) results.push_back(sr);
        }

        // L1 短期记忆（去重）
        if (auto r = short_term_->search(query, top_k)) {
            for (auto& sr : *r) {
                bool dup = false;
                for (auto& existing : results) {
                    if (existing.entry.id == sr.entry.id) { dup = true; break; }
                }
                if (!dup) results.push_back(sr);
            }
        }

        // L2 长期记忆（去重）
        if (auto r = long_term_->search(query, top_k)) {
            for (auto& sr : *r) {
                bool dup = false;
                for (auto& existing : results) {
                    if (existing.entry.id == sr.entry.id) { dup = true; break; }
    ...
    void consolidate(float importance_threshold = 0.6f) {
        // 检索所有工作记忆条目
        if (auto r = working_->search("", 1000)) {
            for (auto& sr : *r) {
                if (sr.entry.importance >= importance_threshold) {
                    long_term_->write(sr.entry);
                }
            }
        }
    }
