...
struct ToolSchema {
    ...
    std::string to_function_json() const {
        ...
        for (auto& p : params) {
            ...
            std::string type_str;
            switch (p.type) {
                case ParamType::String:  type_str = "string"; break;
                case ParamType::Integer: type_str = "integer"; break;
                case ParamType::Float:   type_str = "number"; break;
                case ParamType::Boolean: type_str = "boolean"; break;
                default:                 type_str = "object"; break;
            }
            params_json += fmt::format(R"("{}":{{" type":"{}","description":{}}})",
                p.name, type_str, Json::quote(p.description));
            if (p.required) {
                if (!first_req) required_json += ',';
        ...
        return fmt::format(
            ...
            Json::quote(id), Json::quote(description), params_json);
    }
};
...
inline ParsedArgs parse_args(const std::string& json) {
    ...
    while (pos < json.size()) {
        ...
        if (!key.empty()) args.values[key] = value;
    }
    return args;
}
...
struct ToolResult {
    ...
    static ToolResult ok(std::string out) {
...
public:
    ...
    ToolResult execute(const ParsedArgs& args) override {
        ...
        if (op == "set") {
            ...
            if (it == store_.end()) return ToolResult::fail("Key not found");
...
public:
    explicit ShellTool(std::unordered_set<std::string> allowed_cmds = {
        "echo", "date", "pwd", "ls", "cat", "wc", "sort", "uniq"
    ...
    ToolResult execute(const ParsedArgs& args) override {
        ...
        if (!pipe) return ToolResult::fail("Failed to open pipe");
...
public:
    ...
    ToolResult execute(const ParsedArgs& args) override {
        ...
        if (url.empty()) return ToolResult::fail("URL is required");
        // 安全检查：只允许 http/https
        if (url.substr(0, 4) != "http")
            return ToolResult::fail("Only http/https URLs are allowed");

        std::string cmd = fmt::format(
            "curl -s --max-time 10 --max-filesize 102400 {}", url);
...
public:
    ...
    void register_fn(ToolSchema schema, Fn&& fn) {
    ...
    bool has(const std::string& id) const {
        std::lock_guard lk(mu_); return registry_.count(id) > 0;
    }

    void unregister(const std::string& id) {
        std::lock_guard lk(mu_); registry_.erase(id);
    }
...
public:
    ...
    // 路由并执行来自 LLM 的工具调用请求
    ToolResult dispatch(const kernel::ToolCallRequest& call,
