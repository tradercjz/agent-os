...
| `agentOS::createAgent2(name, ...)` | LONG | 推荐写法：显式 V2 创建持久 Agent |
| `agentOS::ask2(h, q, [sys])` | STRING | 推荐写法：对持久 Agent 发起同步对话 |
| `agentOS::askStream2(h, q, [sys], [cb])` | STRING | 推荐写法：对持久 Agent 发起流式对话 |
