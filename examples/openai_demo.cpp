...
//   硅基流动：OPENAI_BASE_URL=https://api.siliconflow.cn/v1  OPENAI_MODEL=Qwen/Qwen2.5-7B-Instruct
...
static void print_assistant(const std::string& msg) {
    std::cout << "\033[32m助手: " << msg << "\033[0m\n";
}
    ...
    Result<std::string> run(std::string user_input) override {
        ...
        if (auto memories = recall(user_input, 3)) {
            if (!memories->empty()) {
                ...
                for (auto& sr : *memories) mem_ctx += "- " + sr.entry.content + "\n";
...
int main() {
    ...
    // ── 2. 构建 AgentOS ─────────────────────────────────────
    auto backend = std::make_unique<kernel::OpenAIBackend>(api_key, base_url, model);
    ...
    os.tools().registry().register_fn(
        ...
        [](const tools::ParsedArgs& args) -> tools::ToolResult {
            ...
            if (!p) return tools::ToolResult::fail("bc 调用失败");
    ...
    print_info(fmt::format("已注册 {} 个工具: kv_store, shell_exec, http_fetch, calculator",
                           os.tools().registry().list_schemas().size()));

    // ── 4. 创建 Agent ────────────────────────────────────────
    AgentConfig agent_cfg;
    agent_cfg.name          = "Assistant";
    agent_cfg.role_prompt   =
        "你是一个有用的 AI 助手，由 AgentOS 驱动。"
        "你可以使用以下工具帮助用户：\n"
        "- kv_store：存储和读取键值数据\n"
        "- calculator：执行数学计算\n"
